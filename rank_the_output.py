...
def main(args):
    ...
    if args["aug"]:
        o_split_lines = [newlines[600 * i:600 * i + 600] for i in range(0, int(len(newlines) / 600))]
        split_lines = []
        for lines in o_split_lines:
            split_line = [lines[x:x + 60] for x in range(0, 600, 60)]  #split_line [(type1,aug1,beam1),(type1,aug1,beam2)]
            t_split =[]
    ...
    for smiles in tqdm(split_lines):
        ...
        rank_line.append(lo.pop(2))  #top-1 beam3
        rank_line.append(lo.pop(3))
        rank_line.append(lo.pop(3))
        #top-3 beam1
        rank_line.extend(lo)
        if args["aug"]:
