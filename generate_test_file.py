...
def main(args):
    ...
    with open("output/output/test.targets", 'w') as files:
        for i in list(file["Reactants"]):
            files.write(str(i) + "\n")
    e = [smi_tokenizer(canonicalize_and_remove(ss)) for ss in file["Reactants"]]
    f = [smi_tokenizer(canonicalize_and_remove(ss)) for ss in file["Products"]]
    g = [canonicalize_and_remove(ss) for ss in file["Reactants"]]
    h = [canonicalize_and_remove(ss) for ss in file["Products"]]
    file["to_reactants"] = e
    ...
    ll = pd.DataFrame()
    new_sources = ["class" + str(a) + " " + b for a, b in zip(top_10_classes, top_10_targets)]
    ...
    ps.to_csv("output/output/re_index_55_test_out_top10.csv")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--original_file', default="../clustering/scripts/data/cluster_test_50k.csv", type=str)
    parser.add_argument('--split', default="chem", choices=["chem","token"])
